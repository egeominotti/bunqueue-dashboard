...
export interface FlowRootResolution {
  ...
  path: JobFull[];
  /** A legitimate removed ancestor that v2.9.3 can no longer return over HTTP. */
...
/** Climb the complete v2.9.3 parent chain; never claim a partial subtree is the root. */
export async function resolveFlowRoot(
  ...
  for (let hops = 0; ; hops++) {
    ...
    if (hops >= MAX_PARENT_HOPS) {
      throw new FlowSnapshotError(
        `Flow parent chain exceeds the Bunqueue 2.9.3 limit of ${MAX_PARENT_HOPS} levels`
      );
    }
    seen.add(parent);
    current = parent;
