...
export const FLOW_DELETION_UNAVAILABLE =
  'Unavailable in Bunqueue v2.9.2: the server cannot inspect reverse flow dependencies or delete/discard topology atomically.';

export const FLOW_BULK_RETRY_UNAVAILABLE =
  'DLQ retry is unavailable in Bunqueue v2.9.2: its GET and retry POST have no atomic job-generation, state, or flow-topology precondition.';

export const FLOW_DLQ_RETRY_UNAVAILABLE = FLOW_BULK_RETRY_UNAVAILABLE;

export const FLOW_COMPLETED_REQUEUE_UNAVAILABLE =
  'Completed-job requeue is unavailable in Bunqueue v2.9.2 because retry-completed re-inserts a child without rebuilding its flow dependency registration.';
