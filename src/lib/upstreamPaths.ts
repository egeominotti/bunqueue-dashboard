/** Queue grammar accepted by Bunqueue v2.8.55. */
...
/**
 * v2.8.55 reads several resource ids from `URL.pathname` without decoding the
 ...
 * preserves them byte-for-byte in pathname and the v2.8.55 router treats them
...
export function opaqueHttpIdError(id: string): string | null {
  ...
  if (!OPAQUE_HTTP_ID_RE.test(id)) {
    return "Bunqueue v2.8.55 HTTP can address only opaque path-safe IDs (letters, numbers, and . _ ~ ! $ & ' ( ) * + , ; = : @ % [ ] | -); spaces, slash, backslash, ? and # are unsupported";
  }
