...
const WEBHOOK_EVENT_SET = new Set([
  ...
  'job.failed',
  'job.progress',
  // v2.9.2 keeps this legacy value readable in persisted registries even
...
/** `/queues/summary` is a bare array in v2.9.2, so validate it explicitly. */
export function parseQueueSummaryPayload(value: unknown): QueueSummaryFull[] {
  // v2.9.2 returns every queue from this endpoint and defines no upper bound.
...
export function parseWorkersPayload(value: unknown): WorkersPayload {
  ...
  for (let index = 0; index < data.workers.length; index++) {
    ...
    if (!worker) issues.push('entry is not an object');
      ...
      // by the server clock. A valid v2.9.2 worker can therefore be negative.
