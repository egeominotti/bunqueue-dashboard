...
export const queueApi = {
  ...
  setDlqConfig: (queue: string, config: Partial<DlqConfig>) => {
    if (config.autoRetry === true) {
      throw new TypeError(
        'DLQ auto-retry is unavailable: Bunqueue v2.9.3 does not rebuild flow dependency registration.'
      );
    }
  ...
  retryDlq: (_queue: string, _jobId?: string): never => {
    throw new TypeError(
      'DLQ retry is unavailable in Bunqueue v2.9.3 because the endpoint has no atomic job-generation, state, or flow-topology precondition.'
    );
  },
