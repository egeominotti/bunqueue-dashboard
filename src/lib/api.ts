...
export const api = {
  ...
  // Disk-full is a diagnostic state, not a transport failure: bunqueue v2.9.3
  ...
  // Bunqueue 2.9.3 exposes both unauthenticated liveness aliases as plain
  ...
  retryCompleted: (_queue: string): never => {
    throw new TypeError(
      'Completed-job requeue is unavailable in Bunqueue v2.9.3 because flow dependency registration is not rebuilt.'
    );
  },
  ...
  retryDlq: (_queue: string): never => {
    throw new TypeError(
      'DLQ retry is unavailable in Bunqueue v2.9.3 because the endpoint has no atomic flow-safety precondition.'
    );
  },
