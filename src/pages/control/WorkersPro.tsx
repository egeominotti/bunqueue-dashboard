...
export function WorkersPro() {
  ...
  const removeStaleRegistration = async (id: string) => {
    if (
      !window.confirm(
        `Remove the stale registry record for worker "${id}"? This does not stop the worker process. Continue only after confirming that process is stopped; Bunqueue v2.9.0 workers do not automatically re-register after their heartbeat record is removed.`
      )
    )
      return;
      ...
      {quarantinedWorkers.length > 0 && (
        <div role="alert" className="mb-4 rounded-lg border border-danger/30 bg-danger/5 p-4">
          <p className="text-sm font-medium text-danger">
            ...
            2.9.0 but unsafe to render. Healthy workers remain available below.
          </p>
