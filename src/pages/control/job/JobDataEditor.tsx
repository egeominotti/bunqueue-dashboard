...
/**
 * v2.9.3's UpdateJobData replaces the complete payload. FlowProducer stores
...
export function jobDataReadOnlyReason(
  ...
  if (job.parentId || (job.childrenIds?.length ?? 0) > 0 || hasReservedFlowData) {
    return 'Data is read-only for Flow jobs because Bunqueue v2.9.3 replaces the full payload and would remove structural metadata.';
  }
  return null;
}
