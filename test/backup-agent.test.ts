...
describe('Bunqueue S3 backup agent', () => {
  test('rejects malformed status and list data at the executor boundary', async () => {
    ...
    for (const [operation, data] of malformed) {
      ...
      await expect(runner.execute(config, operation)).rejects.toThrow('invalid 2.8.57 contract');
      await runner.close();
    }
  });

  test('normalizes the exact 2.8.57 backup list contract', async () => {
  ...
  test('executes the official 2.8.57 CLI status contract locally', async () => {
