...
2. The page climbs `parentId` to the flow's root, following up to the 100 levels
   supported by the v2.8.55 flow planner.
...
Job IDs must also be addressable by v2.8.55's non-decoding `/jobs/:id` HTTP
...
Every v2.8.55 job snapshot must explicitly contain all three topology fields:
...
Bunqueue v2.8.55 does not expose reverse-dependency inspection or
...
v2.8.55 stores every canonical child in both the parent's `childrenIds` and
