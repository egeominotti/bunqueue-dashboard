...
console for Bunqueue 2.8.59's official `FlowProducer` and Flow Job contracts.
...
`Job.updateProgress` follows both Bunqueue 2.8.59 forms. Numeric progress is
...
2. The page climbs `parentId` to the flow's root, following up to the 100 levels
   supported by the v2.8.59 flow planner.
...
Job IDs must also be addressable by v2.8.59's non-decoding `/jobs/:id` HTTP
...
Every v2.8.59 job snapshot must explicitly contain all three topology fields:
...
Bunqueue v2.8.59 does not expose reverse-dependency inspection or
...
v2.8.59 stores every canonical child in both the parent's `childrenIds` and
