...
Once a queue is selected, a row of eight count cards summarizes every v2.8.55 job state (numbers shown with thousands separators), followed by cards for every control you can operate.
...
**Requeue completed** is visible but disabled. Bunqueue v2.8.55's
...
**Replace rate-limit policy**, enter a positive integer limit and an explicit positive **Window (ms)**, then choose a permanent policy or **Expires after** with a TTL. Bunqueue v2.8.55 has no GET for the existing policy, so this is explicitly a blind desired-state replacement. Success produces a timestamped write receipt, not a claim about current state.
...
2. If upstream auto-retry is already enabled, turn it off. The dashboard never
   permits enabling it because v2.8.55 cannot verify hidden reverse flow
...
**Drain** and **Clean** are visible but disabled. v2.8.55 has no
