...
| Webhooks | Bunqueue 2.9.2 keeps these in the broker process. Configure and inspect them on the intended active node; they are not a PostgreSQL-wide registry. |
...
container, three Bunqueue 2.9.2 brokers, and three authenticated agents. It
...
Bunqueue 2.9.2 uses PostgreSQL schema 19. Stop or coordinate every broker that
shares a namespace before a schema-changing upgrade; an older broker cannot
join after the namespace has migrated beyond its supported schema.
:::
