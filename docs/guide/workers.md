...
::: warning
Registry cleanup **does not stop the worker process**. In Bunqueue v2.9.3, a running worker does not automatically re-register after its heartbeat record is removed. Stop and verify the process first; use this action only for stale, idle records.
:::
