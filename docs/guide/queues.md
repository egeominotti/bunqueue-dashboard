...
::: warning
Pause and resume act **immediately**, there's no confirmation prompt. A single mis-click can pause a live queue, so check the row's **Status** pill afterward. Flow-destructive Drain/Clean/Obliterate controls are visible but disabled on the detail/control pages because v2.8.57 cannot inspect reverse dependencies.
:::
