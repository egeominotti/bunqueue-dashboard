---
title: Job Inspector
description: "Look up a job, inspect its data, result, errors, logs and history, and run only the mutations authorized by state and the v2.8.59 atomicity policy."
---
...
state and the v2.8.59 atomicity policy.
...
Flow members are always read-only. Bunqueue v2.8.59 replaces the entire payload
...
every state because v2.8.59 has no reverse-dependency check or
