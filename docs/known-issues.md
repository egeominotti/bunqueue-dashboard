...
## [Bunqueue v2.9.x](https://github.com/egeominotti/bunqueue/releases/tag/v2.9.3) server-contract constraints
...
- **Rate-limit and concurrency policies are write-only over HTTP.** Bunqueue's
  ...
  Queue Control now complements them with live, target-pinned Bunqueue 2.9.3
...
- **JobDataEditor no longer wipes unsaved edits** on every action-driven job
  ...
  v2.9.3 replaces the full payload and would otherwise erase the reserved
...
- **S3 operations require the local control agent.** `/s3` can now apply the
  ...
  and perform a stop-gated, snapshot-confirmed restore through the exact 2.9.3
