...
| `/add-job` | `AddJob` | Enqueue with every option the v2.9.3 HTTP single-push route forwards, including tags/group/dependencies, structured backoff and interval repeat policy; single or repeated copies via a `Count` field (validated, ≤10000). |
...
| `/cron` | `CronManager` | Create a schedule (cron expression *or* interval-in-ms, live next-runs preview) with v2.9.3 dedup and complete spawned-job options, then list/delete schedules. |
| `/dlq-control` | `DlqControl` | Single-queue DLQ triage. Removal, row retry, Retry-all and Purge are unavailable under the v2.9.3 atomicity policy. |
