...
| `/add-job` | `AddJob` | Enqueue with the complete v2.8.55 single-push options, including tags/group/dependencies, structured backoff and repeat policy; single or repeated copies via a `Count` field (validated, ≤10000). |
| `/jobs/bulk-add` | `BulkAddJobs` | Bulk enqueue: paste a JSON array or NDJSON (one JSON value per line). Spec mode preserves the full v2.8.55 bulk `JobInput` surface and maps `jobId`/`customId` correctly; raw mode keeps the whole item as data. |
...
| `/cron` | `CronManager` | Create a schedule (cron expression *or* interval-in-ms, live next-runs preview) with v2.8.55 dedup and complete spawned-job options, then list/delete schedules. |
| `/dlq-control` | `DlqControl` | Single-queue DLQ triage. Row retry, Retry-all and Purge are unavailable under the v2.8.55 atomicity policy. |
