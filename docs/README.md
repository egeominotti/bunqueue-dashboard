...
- [pages.md](pages.md), every route, exactly which component it renders, which API client it uses, and what it does. Start here if you're not sure
  which page owns a piece of functionality, several routes render a
  different page family than their path would suggest.
- [components.md](components.md), the UI kit, layout shell, Zustand stores, and shared `lib/` helpers that every page is built from.
...
- [api-mapping.md](api-mapping.md), endpoint map, request bodies, **verified response-shape gotchas**, the job-action state-gating table, and
...
v2.8.57 server contract (`lib/jobActions.ts`). DLQ retry and completed-job
